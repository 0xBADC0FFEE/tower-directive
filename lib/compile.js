...
function compile(element, ctx) {
  return isArray(element)
    ? compileEach(element, ctx)
    : compileEach([element], ctx);
}

function compileEach(children, ctx) {
  var node, directives;
  for (var i = 0, n = children.length; i < n; i++) {
    node = children[i];
    directives = compileNode(node);
    if (directives.length) exec(node, directives, ctx);
    // recursive
    if (node.childNodes) compileEach(node.childNodes, ctx);
  }
}
