...
function directives(directive) {
  ...
  // XXX: extract out to separate module, add ability to do multi-key commands (mousetrap)
  // <input on-enter="createTodo">
  function keyboardDirective(key, name) {
    name || (name = 'keypress');
    directive('on-' + key, function(scope, element, attr){
      function handle(evt) {
        if (key === keyname[which(evt)])
          scope.apply(attr.value, [evt]);
      }
